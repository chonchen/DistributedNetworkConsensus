...
public class Simulation {
    ...
    public static void main(String[] args) {
        ...
        int numNodes = 100;
        double p_graph = Double.parseDouble(args[0]); // parameter for random
                                                      // graph: prob. that an
                                                      // edge will exist
        double p_malicious = Double.parseDouble(args[1]); // prob. that a node
                                                          // will be set to be
                                                          // malicious
        double p_txDistribution = Double.parseDouble(args[2]); // probability of
                                                               // assigning an
                                                               // initial
                                                               // transaction to
                                                               // each node
        int numRounds = Integer.parseInt(args[3]); // number of simulation
                                                   // rounds your nodes will run
                                                   // for
        ...
        for (int i = 0; i < numNodes; i++) {
            if (Math.random() < p_malicious)
                // When you are ready to try testing with malicious nodes,
                // replace the
        ...
        boolean[][] followees = new boolean[numNodes][numNodes]; // followees[i][j]
                                                                 // is true iff
                                                                 // i follows j
        for (int i = 0; i < numNodes; i++) {
        ...
        // the starting state of Transactions each node has heard. The
        // distribution
        // is random with probability p_txDistribution for each Transaction-Node
        // pair.
        for (int i = 0; i < numNodes; i++) {
            HashSet<Transaction> pendingTransactions = new HashSet<Transaction>();
            for (Integer txID : validTxIds) {
                if (Math.random() < p_txDistribution) // p_txDistribution is
                                                      // .01, .05, or .10.
        ...
        for (int round = 0; round < numRounds; round++) { // numRounds is either
                                                          // 10 or 20

            // gather all the proposals into a map. The key is the index of the
            // node receiving
            // proposals. The value is an ArrayList containing 1x2 Integer
            // arrays. The first
            // element of each array is the id of the transaction being proposed
            // and the second
            ...
            // Distribute the Proposals to their intended recipients as
            // Candidates
            for (int i = 0; i < numNodes; i++) {
